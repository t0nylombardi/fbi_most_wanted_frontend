...
// Mock data
const mockPerson: WantedPerson = mockedPersons[0];
const mockUpdatedDetails: PersonDetails = mockedDetails;
...
describe("wanted service", () => {
  ...
  describe("create", () => {
    it("should create a wanted person", async () => {
      ...
      (fetchData as jest.Mock).mockResolvedValue({ data: mockPerson });
  ...
  describe("update", () => {
    it("should update a wanted person", async () => {
      ...
      expect(fetchData).toHaveBeenCalledWith(
        `/default_all_pages/${mockPerson.id}`,
        "PUT",
        mockUpdatedDetails,
      );
    ...
    it("should return null on error", async () => {
      (fetchData as jest.Mock).mockRejectedValue("Update failed");
